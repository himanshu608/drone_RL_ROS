...
from drone_training.msg import Altimeter
...
class QuadCopterEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.takeoff_pub = rospy.Publisher(
            '/drone/takeoff', EmptyTopicMsg, queue_size=0)
    ...
    def take_observation(self):
        ...
        while data_pose is None:
            try:
                data_pose = rospy.wait_for_message(
                    '/drone/gt_pose', Pose, timeout=5)
            except:
                rospy.loginfo(
    ...
    def process_data(self, data_position, data_imu):
        ...
        if altitude_bad or pitch_bad or roll_bad:
            rospy.loginfo("(Drone flight status is wrong) >>> (" +
                          str(altitude_bad)+","+str(pitch_bad)+","+str(roll_bad)+")")
            done = True
            reward = -200
        else:
